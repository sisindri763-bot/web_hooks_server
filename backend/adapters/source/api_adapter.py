"""
adapters/source/api_adapter.py
--------------------------------
REST API source adapter — any JSON endpoint.
size_bytes = Content-Length from response headers.

Required config keys:
    url
Optional:
    method, headers, params, body, data_key, sample_rows, timeout
"""
...
class APISourceAdapter(DataAdapter):
    role = "source"
    ...
    def fetch_snapshot(self, config: Dict[str, Any], run_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Derive a readable object name from the URL path
